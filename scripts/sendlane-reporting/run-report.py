...
def process_line(line,topic):
    try:
        ...
        if topic == 'email_sent_log':
            ...
            send_datetime = timezoneobject.localize(datetime.fromisoformat(data['sent_time']['date']))
            send_date = send_datetime.strftime('%Y-%m-%d')
...
def display_stats():
    global stats
    stats_rows = []
    print("\t".join(['report_date','domain','job_id','send_date','sent','delivered','block','hard_bounce','soft_bounce','spam_bounce','feedback','open','click','unsubscribe','unique_hard_bounce','unique_soft_bounce','unique_spam_bounce','unique_feedback','unique_open','unique_click','unique_unsubscribe']))
    for domain in stats.keys():
        for job_id in stats[domain].keys():
            for send_date in stats[domain][job_id].keys():
                ...
                print(f"{report_date}\t{domain}\t{job_id}\t{send_date}\t" + "\t".join(send_stats) + "\t" + "\t".join(unique_stats))
                stats_rows.append([str(report_date),domain,job_id,str(send_date),*send_stats,*unique_stats])
    return
